...
class PayPalIntegrationTest {
  ...
  async testEnvironmentConfig() {
    ...
    const tests = [
      ...
      },
      {
        ...
        warning: process.env.JWT_SECRET?.includes("amazingsecret")
          ? "Using development secret - change for production!"
          : null,
      },
      {
    ...
    try {
      ...
      if (response.ok && order.id) {
        ...
        if (approveLink) {
          ...
          logWarning(
            "Note: In production, customer would be redirected to this URL to complete payment",
          );
        }
  ...
  async testDatabaseSchema() {
    ...
    try {
      ...
      try {
        ...
        logInfo(
          "Required fields present: productVariant, downloadToken, expiresAt, maxDownloads",
        );
        this.results.passed++;
        this.results.tests.push({
          category: "Database",
  ...
  generateReport() {
    ...
    for (const category of categories) {
      log(`\n${category}:`, "cyan");
      const categoryTests = this.results.tests.filter(
