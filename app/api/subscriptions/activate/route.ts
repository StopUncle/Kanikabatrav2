  ...
  return requireAuth(request, async (_req, user) => {
    try {
      ...
      const paypalSubscription =
        await paypalSubscriptionService.getSubscription(subscriptionId);

      if (
        paypalSubscription.status !== "ACTIVE" &&
        paypalSubscription.status !== "APPROVED"
      ) {
        return NextResponse.json(
          {
            success: false,
            error: `Subscription is not active. Status: ${paypalSubscription.status}`,
          },
          { status: 400 },
        );
      }
