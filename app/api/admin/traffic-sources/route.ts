...
export async function GET(request: NextRequest) {
  ...
  const [
    ...
    prisma.user.count({ where: { createdAt: { gte: since }, isBot: false } }),
    prisma.user.groupBy({
      by: ["utmSource"],
      where: { createdAt: { gte: since }, utmSource: { not: null }, isBot: false },
      _count: { _all: true },
      orderBy: { _count: { utmSource: "desc" } },
    }),
    prisma.user.groupBy({
      by: ["utmCampaign"],
      where: { createdAt: { gte: since }, utmCampaign: { not: null }, isBot: false },
      _count: { _all: true },
    ...
    prisma.user.groupBy({
      by: ["ipCountry"],
      where: { createdAt: { gte: since }, ipCountry: { not: null }, isBot: false },
      _count: { _all: true },
