  ...
  return (
    <div className="space-y-3">
      {posts.map((post) => (
        <Link
          key={post.id}
          href={`/consilium/forum/${categorySlug}/post/${post.slug}`}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-1">
                {post.isPinned && <Pin className="w-4 h-4 text-accent-gold" />}
                {post.isLocked && <Lock className="w-4 h-4 text-text-muted" />}
