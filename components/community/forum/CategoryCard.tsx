...
export default function CategoryCard({ category }: CategoryCardProps) {
  const isPremium =
    category.accessTier === "PREMIUM" ||
    category.accessTier === "COACHING_CLIENT";

  return (
    <Link
      href={category.hasAccess ? `/consilium/forum/${category.slug}` : "#"}
      className={`
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          >
            {category.icon ? (
              ...
              <MessageSquare
                className={`w-6 h-6 ${category.hasAccess ? "text-accent-gold" : "text-gray-500"}`}
              />
            )}
          </div>
          <div>
            <h3
              className={`font-semibold ${category.hasAccess ? "text-white" : "text-gray-400"}`}
            >
              {category.name}
            </h3>
        ...
        <div className="flex items-center gap-2">
          {isPremium && <Crown className="w-5 h-5 text-accent-gold" />}
          {!category.hasAccess && <Lock className="w-5 h-5 text-gray-500" />}
        </div>
      </div>
