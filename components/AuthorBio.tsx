...
export default function AuthorBio({
  ...
  if (variant === "full") {
    return (
      <div
        className={`bg-gradient-to-br from-deep-black/80 to-deep-navy/40 border border-white/10 rounded-2xl p-8 lg:p-10 ${className}`}
      >
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-grow">
            <p className="text-accent-gold text-xs uppercase tracking-[0.2em] mb-2">
              About the Author
            </p>
            ...
            <p className="text-text-light leading-relaxed mb-6">
              {SITE_CONFIG.description} With{" "}
              {SOCIAL_METRICS.combined.totalFollowers} followers and{" "}
              {SOCIAL_METRICS.combined.totalViews} views across platforms,
              Kanika offers a rare first-person perspective on psychology that
              most can only theorize about.
            </p>
  ...
  return (
    <div
      className={`flex items-start gap-5 p-6 bg-gradient-to-br from-deep-black/80 to-deep-navy/40 border border-white/10 rounded-xl ${className}`}
    >
      ...
      <div>
        ...
        <p className="text-sm text-text-gray leading-relaxed">
          Clinically diagnosed with ASPD, Kanika provides unique first-person
          insights into dark psychology and Cluster B personality disorders.
        </p>
      </div>
    </div>
  );
