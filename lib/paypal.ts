...
class PayPalService {
  ...
  private initialize() {
    ...
    console.log(
      `PayPal Service initialized: ${environment} environment (${this.baseURL})`,
    );
  }
  ...
  async getAccessToken(): Promise<string> {
    ...
    console.log(
      `Client ID (first 20 chars): ${this.clientId!.substring(0, 20)}...`,
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `PayPal auth failed: ${error.error_description || response.statusText}`,
      );
    }
 ...
 */
export function createBookOrder(
  bookPrice: number,
  bookTitle: string,
): PayPalOrder {
  return {
        ...
        items: [
          {
            ...
            description:
              "Detailed personality analysis with radar chart, percentages, and insights delivered to your email",
            quantity: "1",
            unit_amount: {
              currency_code: PAYPAL_CONFIG.currency,
              value: quizPrice.toFixed(2),
...
/**
 * Create order for Ask Kanika question
 */
export function createAskKanikaOrder(
  price: number,
  format: string,
  questionCount: number,
): PayPalOrder {
  const name = `Ask Kanika - ${format === "voice" ? "Voice" : "Written"} Answer (${questionCount} question${questionCount > 1 ? "s" : ""})`;
  return {
    intent: "CAPTURE",
    purchase_units: [
      {
        reference_id: "ask-kanika",
        description: name,
        amount: {
          currency_code: PAYPAL_CONFIG.currency,
        ...
        items: [
          {
            name,
            description: `Personal ${format} response from Kanika Batra`,
            quantity: "1",
            unit_amount: {
              currency_code: PAYPAL_CONFIG.currency,
              value: price.toFixed(2),
